...
* nginx 
  * 웹 서버, 리버스 프록시, 캐싱, 로드 밸런싱, 미디어 스트리밍을 위한 오픈소스 소프트웨어
  * 리버스 프록시는 ginx가 외부의 요청을 받아 백엔드 서버로 요청을 전달하는 행위로 실제 요청에 대한 처리는 뒷단의 웹 애플리케이션 서버들이 처리한다.
* [엔진엑스와 스프링 부트 연동 및 무중단 배포스크립트 작성과 무중단 배포](https://chobo24.tistory.com/entry/24%EC%8B%9C%EA%B0%84-365%EC%9D%BC-%EC%A4%91%EB%8B%A8-%EC%97%86%EB%8A%94-%EC%84%9C%EB%B9%84%EC%8A%A4-%EB%AC%B4%EC%A4%91%EB%8B%A8-%EB%B0%B0%ED%8F%AC)
